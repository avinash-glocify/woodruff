...
class GoogleAnalyticsFilterImport implements ToCollection,WithHeadingRow
{
    public $project;
    ...
    public function getMetaData($row, $existDetail)
    {
        $data = [
          ...
          'ga_page_path'                          => $row['ga_page_path'] ?? ( $existDetail ? $existDetail->ga_page_path : null ),
          ...
          'losing_traffic'                        => $row['losing_traffic'] ?? ( $existDetail ? $existDetail->losing_traffic : null ),
        ];
        return $data;
    }
}
